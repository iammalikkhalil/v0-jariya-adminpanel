...
import { createZikrReward, updateZikrReward, type ZikrReward, type CreateZikrRewardRequest } from "@/lib/api/reward"
import { getAllZikrs, type Zikr } from "@/lib/api/zikr"

interface ZikrRewardFormProps {
  reward?: ZikrReward
  mode: "create" | "edit"
}

export function ZikrRewardForm({ reward, mode }: ZikrRewardFormProps) {
  const router = useRouter()
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState("")
  const [zikrs, setZikrs] = useState<Zikr[]>([])

  const [formData, setFormData] = useState<CreateZikrRewardRequest>({
    zikrId: reward?.zikrId || "",
    text: reward?.text || "",
  })

  // Set initial form data when reward prop changes
  useEffect(() => {
    if (reward) {
      setFormData({
        zikrId: reward.zikrId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = mode === "create" 
        ? await createZikrReward(formData) 
        : await updateZikrReward({ ...formData, id: reward!.id })

      if (response.success) {
        router.push("/admin/rewards-qualities/rewards")
      } else {
        setError(response.message || `Failed to ${mode} reward`)
      }
    } catch (err) {
  ...
  const handleInputChange = (field: keyof CreateZikrRewardRequest, value: string) => {
    ...
    <Card className="max-w-2xl mx-auto">
      <CardHeader>
        <CardTitle className="font-islamic">
          {mode === "create" ? "Add New Zikr Reward" : "Edit Zikr Reward"}
        </CardTitle>
        <CardDescription>
          {mode === "create" 
            ? "Create a new reward for a zikr" 
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="text" className="text-sm font-medium">
              Reward Text <span className="text-destructive">*</span>
            </Label>
            <Textarea
              id="text"
              value={formData.text}
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {isLoading ? "Saving..." : mode === "create" ? "Create Reward" : "Update Reward"}
            </Button>
            <Button 
              type="button" 
              variant="outline" 
